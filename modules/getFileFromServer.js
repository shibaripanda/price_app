...
export default async function getFileFromServerPogremuhi(nameFile, limit){
    return new Promise(async function(resolve, reject) {
        ...
        for (let i = 0; i < limit; date.setDate(date.getDate() - 1)){
            console.log('Test: ' + date.toLocaleDateString("en-GB").split('/').join('.'))
            const res = await fetch('http://www.pogremuhi.com/pricelist/' + date.toLocaleDateString("en-GB").split('/').join('.') + '.xls')
